...
inductive Ctx.InnerTy : Ctx → Tm → Tm → Prop
  ...
  | pair_cf {Γ : Ctx} {m n : ℕ} {A B a b : Tm} {L : Finset ℕ}
    ...
    → HasTy Γ A a
    → HasTy Γ (B.bs0 a) b
    → InnerTy Γ (.sigma A B) (.pair A B a b)
  ...
  | succ {Γ : Ctx} : Γ.Ok → InnerTy Γ (.pi .nats .nats) .succ
...
theorem Ctx.HasTy.unique_helper {Γ : Ctx} {X Y a : Tm} (hA : Γ.HasTy X a) (hB : Γ.InnerTy Y a)
  : TyEq' Γ X Y := by induction hA generalizing Y with
  | univ | unit | nil | empty | eqn =>
    cases hB; apply TyEq'.of_ty; constructor; constructor; apply Ctx.Ok.zero
            ; first | assumption | apply HasTy.ok; assumption
  | pi_cf hA hB hℓ IA IB => cases hB with | pi_cf hA' hB' hℓ' =>
    have ⟨C, hC, eC⟩ := hA'.outer_ty
    have IC := IA hC
    sorry
  | cast hAB ha IA => exact .head hAB.symm (IA hB)
  | _ => sorry
